...
- **GUNCELLEME (2026-08-14, baslatma-oncesi 3-ajanli denetim):** (i) Ucuncu bolme **V_C 2k (negatif kontrol)** eklendi → D_core 44k; V_B-vs-V_C saf secim-gurultusu tabanini olcer (iki 2k bolme arasi fark SE'si ~1.55 puan — sizinti sifirken bile argmax gurultuyle donebilir; V_C olmadan Δ atfedilemezdi). (ii) Analiz kurallari egitimden ONCE sabitlendi: `E2_ISTATISTIK_PROTOKOLU.md` — **McNemar es-birincil'e yukseltildi** (3 tohum havuzlanmis ~30k eslesmis birim; n=3 TOST'unun s_Δ<0.59 toleransina rehin degil), dejenere s_Δ=0 kurali, ortak-bolme sinirlamasi. (iii) Secim ciktilari artik tam-test degerlendirmesi icerir (`--test-eval`: 10k clean+PGD-10 + ornek maskeleri npz). (iv) Butce ~19-28 GPU-saat (patience 0 → tum AT'ler tam 100 epok; +18 secim gecisi ~2-3 sa + test degerlendirmeleri ~1 sa).
