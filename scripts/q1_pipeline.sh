...
#   STAGE=e2       bash scripts/q1_pipeline.sh   # sizinti ablasyonu (21 GPU-sa)
...
clean_args() {
    case "$1" in
        resnet18|resnet50) echo "--lr 0.1 --epochs 200 --batch-size 128" ;;
        vit_tiny)          echo "--lr 0.001 --epochs 200 --batch-size 128" ;;  # C1 klonu
        vit_small)         echo "--lr 0.001 --epochs 30 --batch-size 64 --timm-pretrained" ;;
    esac
}
at_args() {
    case "$1" in
        resnet18|resnet50) echo "--lr 0.001 --epochs 100 --batch-size 128 --patience 20" ;;
        vit_tiny)          echo "--lr 0.001 --epochs 100 --batch-size 64 --patience 20" ;;
        # ViT-S sigortalari (rapor 5.3): LR 5e-4, clip 1.0, 10ep eps-warmup,
        # 5ep lr-warmup, bs 64, 60 epoch butce
        vit_small)         echo "--lr 0.0005 --epochs 60 --batch-size 64 --patience 20 --grad-clip 1.0 --eps-warmup 10 --lr-warmup 5" ;;
    esac
}
...
e2)  # Sizinti ablasyonu: tam butce (patience KAPALI), her epoch ckpt, tek egitim
    run_step Q1_e2_split data/e2_val_B.json python scripts/q1_make_e2_split.py
    for seed in 1001 1002 1003; do
        ...
        run_train "Q1_e2_at_resnet18_${seed}" "${root}/resnet18/adv/adversarial_training" \
            python -m cli.main train adversarial -m resnet18 --dataset cifar10 \
                --lr 0.001 --epochs 100 --batch-size 128 --patience 0 \
                --seed "$seed" -o "$root" \
    ...
    for seed in 2001 2002 2003; do
        ...
        run_train "Q1_e2_at_vit_tiny_${seed}" "${root}/vit_tiny/adv/adversarial_training" \
            python -m cli.main train adversarial -m vit_tiny --dataset cifar10 \
                --lr 0.001 --epochs 100 --batch-size 64 --patience 0 \
                --seed "$seed" -o "$root" \
                --pretrained "${root}/vit_tiny/clean/last.pth" \
                --val-indices data/e2_exclude_for_at.json --save-every 1 \
                --eps $EPS8
    done
    # Cevrimdisi secim: her yorungeye iki kural (V_A sizintili / V_B temiz)
