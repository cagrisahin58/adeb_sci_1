...
ax.plot(xv, vit_cos, "o-", color=C_VIT, label=f"ViT-Tiny (12 blocks)")
ax.plot(xr, res_cos, "s--", color=C_CNN, label=f"ResNet-18 (8 residual blocks)")
imin = int(np.argmin(vit_cos))
ax.annotate("min 0.955 (Block 8)\nthen mild recovery", xy=(xv[imin], vit_cos[imin]),
            xytext=(0.45, 0.975), fontsize=8,
            arrowprops=dict(arrowstyle="->", lw=0.8))
ax.annotate("monotonic decline\nto 0.913", xy=(xr[-1], res_cos[-1]), xytext=(0.62, 0.935),
            fontsize=8, arrowprops=dict(arrowstyle="->", lw=0.8))
ax.set_xlabel("Relative depth (block index / total blocks)")
ax.set_ylabel("Clean–adversarial cosine sim.")
