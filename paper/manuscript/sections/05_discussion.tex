...
Every measurement in this paper is made inside a single threat model: $\Linf$ at $\eps = 8/255$. Whether the protocol spread survives a change of norm is a separate question, and one we have registered but not yet answered. It is worth stating what such a test could and could not show: our models are $\Linf$-trained, so evaluating them under an $L_2$ budget probes the measurement axis rather than $L_2$ robustness, and the resulting absolute values would not be comparable with $L_2$-trained references.
