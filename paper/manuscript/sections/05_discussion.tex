...
The central result is an ordering of variances. Under our protocol, re-running an attack with a different random initialization moves robust accuracy by about $0.05$ points; retraining a model from scratch moves it by at most $0.55$; changing only the conditioning protocol moves the measured transfer asymmetry by $10.45\pm0.76$ points. A quantity whose measurement choice contributes twenty times more variance than the training run is not a stable property of the architectures, and reporting it without the protocol communicates almost nothing. This is not an argument against conditioning---conditioning on correctly classified samples is established practice~\cite{liu2017delving, dong2018boosting, ravikumar2023trend} and remains necessary---but an argument that the choice belongs in the result, not in a footnote.
