...
\paragraph{The direction is stable only while the models are unmatched}
...
\item \textbf{Do not report unconditioned transfer rates as architecture comparisons.} When targets differ in clean accuracy, the raw rate is a known mixture of transferability and target weakness: $r_{\text{raw}} - r_{\text{cond}} = e\,(1 - r_{\text{cond}})$, an identity rather than a fitted relation, so the correction can be applied to published raw rates without re-running anything.
